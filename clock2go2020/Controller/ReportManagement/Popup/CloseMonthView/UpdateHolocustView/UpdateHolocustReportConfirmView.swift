...
class UpdateHolocustReportConfirmView: UIViewController {
    ...
    @IBOutlet weak var revachaView: HolocustReportView!
    @IBOutlet weak var revachaRemarkTextField: UITextField!
    @IBOutlet weak var shouldAddRemarkLabel: UILabel!
    
    var viewModel = UpdateHolocustReportConfirmViewModel()
    weak var delegate: UpdateHolocustReportConfirmViewDelegate?
    ...
    func revachaTaps() {
        ...
        revachaView.therapyTapped = { [weak self]  in
            self?.chooseTherapy()
        }
    ...
    func setupValues() {
        ...
        revachaView.isHidden = false//!viewModel.isHolocust
        revachaRemarkTextField.isHidden = !viewModel.isHolocust
...
//            delegate?.updateDayReport(viewModel.getReport())
            print("report:", viewModel.getReport())
        }
...
extension UpdateHolocustReportConfirmView: ChooseTaskDelegate {
...
protocol UpdateHolocustReportConfirmViewDelegate: NSObjectProtocol {
    func updateDayReport(_ report: EmpDayReportsObj)
}

extension UpdateHolocustReportConfirmView {
    ...
    func chooseTrnsType() {
        ...
        chooseVC.choosedType = { index, title, taskitem in
            ...
            self.viewModel.setTrnsType(index)
            self.viewModel.selectedTrasType = index
            self.setupValues()
        }
    ...
    func chooseTherapy() {
        ...
        let width = revachaView.therapyView.frame.size.width

        let viewModel = SortingListViewModel(type: .holocustTherapy, trasType: self.viewModel.selectedTrasType)
        chooseVC.configure(viewModel: viewModel, top: top.y , left: left, width: width)
//        chooseVC.viewModel?.selectedHolocustTransType = self.viewModel.selectedTrasType
        
        chooseVC.choosedType = { index, title, taskitem in
            self.revachaView.therapyTitle.text = title
//            let task = TaskListViewModel().taskListItems[index].task
            self.viewModel.setTheraphyType(index)
            self.viewModel.selectedTheraphyType = index
            self.setupValues()
        }
        present(chooseVC, animated: true, completion: nil)
    }
    ...
    func chooseEvent() {
        ...
        let viewModel = SortingListViewModel(type: .holocustEvent, trasType: self.viewModel.selectedTrasType, therapyType: self.viewModel.selectedTheraphyType)
//        chooseVC.viewModel?.selectedHolocustTherapyType = self.viewModel.selectedTheraphyType
//        chooseVC.viewModel?.selectedHolocustTransType = self.viewModel.selectedTrasType
...
extension UpdateHolocustReportConfirmView: UITextFieldDelegate {
