...
class UpdateHolocustReportConfirmViewModel {
    ...
    var selectedTrasType = 3
    var selectedTheraphyType = Int()
    ...
    func setTheraphyType(_ index: Int) {
        report.trnsType = index
        if report.extraFields == nil {
            report.extraFields = [:]
        }
        report.extraFields?["TherapyType"] = index
    }
