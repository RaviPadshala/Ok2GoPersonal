...
class ConfirmTaskViewModel {
    ...
    func getAdditionalColor() -> UIColor {
        if  let additionalButtons = CompaniesDataManager.shared.getAddonButtons() {
            ...
            let additionalButton7ActionType = additionalButtons.button_7?.action_type
            let additionalButton8ActionType = additionalButtons.button_8?.action_type

            if additional?.action_type == additionalButton1ActionType || additional?.action_type == additionalButton3ActionType || additional?.action_type == additionalButton5ActionType || additional?.action_type == additionalButton7ActionType {
                return #colorLiteral(red: 0.2101188302, green: 0.7993369699, blue: 0.4015711546, alpha: 1)
            } else if additional?.action_type == additionalButton2ActionType || additional?.action_type == additionalButton4ActionType || additional?.action_type == additionalButton6ActionType || additional?.action_type == additionalButton8ActionType {
